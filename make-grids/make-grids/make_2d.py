...
def regrid_model(inner_boundary: float = 30, outer_boundary: float = 4000):
    ...
    Parameters
    ----------
    inner_boundary: float [optional]
        The inner boundary of the grid in Rg. Default is 30 Rg.
    outer_boundary: float [optional]
        The outer boundary of the grid in Rg. Default is 4000 Rg.
    """
    ...
    if inner_boundary is None:
        ...
        rg_rmin = inner_boundary

    if outer_boundary is not None:
        rg_rmax = outer_boundary
    else:
...
def setup_grid_boundaries(grid: Table):
    ...
    ndim = np.max(grid["j"]) + 1

    # The 2 outer ghost layers
    for i in range(1, 2 * ndim + 1):
        grid[-i]["inwind"] = NOT_INWIND
        grid[-i]["rho"] = 0
        if OUTPUT_TEMPERATURE:
            grid[-i]["t_e"] = grid[-i]["t_r"] = 0

    # The inner ghost layer
    for i in range(ndim):
        grid[i]["inwind"] = NOT_INWIND
        grid[i]["rho"] = 0
        if OUTPUT_TEMPERATURE:
    ...
    for i in range(len(grid)):
        ...
        if grid[i]["theta"] < 5 or grid[i]["theta"] >= 90:
            grid[i]["inwind"] = NOT_INWIND
            grid[i]["rho"] = 0
            if OUTPUT_TEMPERATURE:
                grid[i]["t_e"] = grid[i]["t_r"] = 0

        # This is a bit of a hack to remove regions of the grid where v > c
        # (usually this is the jet region) this should only be a problem if
        # there are cells with r < 2 Rg
        v = np.sqrt(grid[i]["v_x"] ** 2 + grid[i]["v_y"] ** 2 + grid[i]["v_z"] ** 2)
        if v > constants.C:
            grid[i]["inwind"] = NOT_INWIND
            grid[i]["rho"] = 0
            if OUTPUT_TEMPERATURE:
...
if __name__ == "__main__":
    setup_script()
    original_grid, new_grid = regrid_model(inner_boundary=30)  # paper is 30 rg
