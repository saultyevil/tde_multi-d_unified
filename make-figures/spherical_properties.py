...
files = {
    "Bin 1": [
        "../other-data/Envelope_bin1.txt",
        "../simulations/benchmark/Bin-1/Bin-1.txt",
    ],
    "Bin 2": [
        "../other-data/Envelope_bin2.txt",
        "../simulations/benchmark/Bin-2/Bin-2.txt",
    ],
    "Bin 3": [
        "../other-data/Envelope_bin3.txt",
        "../simulations/benchmark/Bin-3/Bin-3.txt",
    ],
    "Bin 4": [
        "../other-data/Envelope_bin4.txt",
...
for n, (theta, (roth_model, me)) in enumerate(files.items()):
    ...
    ax[0].plot(
        me["r"][1:-2] / rg, me["rho"][1:-2], "-", color=f"C{n}", label=f"Bin {n+1}"
    )
    ax[0].plot(roth_model["r_in"] / rg, roth_model["rho"], "--", color=f"C{n}")

    ax[1].plot(
        me["r"][1:-2] / rg,
        me["v_r"][1:-2] / pypython.constants.VLIGHT,
        "-",
        color=f"C{n}",
    )
    ax[1].plot(
        roth_model["r_in"] / rg,
        roth_model["v_in"] / pypython.constants.VLIGHT,
        "--",
        color=f"C{n}",
    )

ax[0].legend()
ax[0] = pypython.plot.set_axes_scales(ax[0], "loglog")
ax[1] = pypython.plot.set_axes_scales(ax[1], "logx")
